---
name: kali-ctf
description: "基于 frpctl 控制面的统一 Kali CTF 技能。用于任何 CTF 题目求解：将本地题目附件和题目 URL 作为输入，本地仅负责 nginx 文件服务器上传与 frpctl 隧道控制，所有侦察、分析、利用和取 flag 全程通过远端 Kali API 执行。运行前先读取当前工作区的 setting.md。"
---
...
2. 仅允许本地执行：`nginx 文件服务器上传` 与 `frpctl` 隧道命令。
...
8. 必须先做设备选择；若目标设备不是后端云服务器，AI 必须停机交接，不得继续自行计算。

## 设备选择与停机交接（强制）

可选设备：
1. `backend-cloud`：后端云服务器（`Api_Base` 对应 Kali 容器，默认设备）。
2. `i9-12900k`：你的本地高性能工作站。
3. `gpu-cloud`：高性能 GPU 云主机（固定环境：`CUDA 13.0` + `Python 3.12` + `Ubuntu 22.04`）。

选择规则：
1. 常规 Web/Pwn/Reverse/Forensics/Misc 题流程：优先 `backend-cloud`。
2. 需要大量 GPU 计算（如大规模 hashcat、CUDA 模型推理、长时密码爆破）：选 `gpu-cloud`。
3. 需要本地专有工具链/硬件特性且后端无同等环境：选 `i9-12900k`。

停机交接规则（必须执行）：
1. 当目标设备不是 `backend-cloud` 时，AI 不继续自动解题。
2. AI 只允许做“交接准备”：
- 生成完整待运行脚本（必须包含环境安装/校验等前置步骤）。
- 打包输入文件、字典、参数、预期输出说明。
- 上传交接包到文件服务器（`File_Base`）。
- 给出一键后台运行命令（避免 SSH 不稳中断）和监控命令。
3. 交接完成后必须停止，等待用户回传结果（日志/输出文件）再继续。

后台运行与监控规范（必须给出）：
1. 后台运行命令必须包含：`nohup` + 日志重定向 + PID 文件。
2. 监控命令至少包含：
- 查看进程：`ps -fp $(cat run.pid)`
- 实时日志：`tail -f run.log`
- 结束任务：`kill $(cat run.pid)`
3. 参考模板：
...
交接输出最小模板（必须给出）：
1. `目标设备`：`i9-12900k` 或 `gpu-cloud`
2. `下载包 URL`：`<File_Base>/<handoff-bundle>`
3. `一键后台运行命令`：可直接复制运行（`nohup ... & echo $! > run.pid`）
4. `监控命令`：`ps/tail/kill` 三条
5. `预计耗时`：粗略区间
6. `回传内容`：需要你回传的文件/日志路径

## 本机命令白名单与禁令（强制）

本机只允许两类命令：
1. `nginx` 文件服务器上传命令（如 `scp`/`rsync` 上传到 `/srv/ctf-files/`）
2. `frpctl` 穿透命令（不手写 frpc 配置，不直接起 frpc 进程）
...
```bash
frpctl quick -l <LOCAL_PORT> -t tcp
# 如需固定远端端口:
frpctl quick -l <LOCAL_PORT> -t tcp --remote-port <REMOTE_PORT>
```
...
1. “接下来除 nginx 文件上传/frpctl 外，本机不再执行任何解题命令。”
...
2. `强制自检`：执行并通过“强制自检环节”。
3. `设备判定`：按“设备选择与停机交接”规则决定目标设备。
4. `分支A`（目标=`backend-cloud`）：继续后续远端自动解题流程。
5. `分支B`（目标!=`backend-cloud`）：执行交接准备、上传交接包、输出执行指引后停止等待用户。
6. `本地传输`：上传附件到 nginx 文件服务器并得到 `File_Base/文件名` 直链。
7. `远端落地`：通过 `/api/kali/exec` 在 Kali 下载到 `/tmp/workspace/current` 并校验。
8. `源文件清理`：下载校验成功后，立即删除 `/srv/ctf-files/<文件名>`。
9. `题型判定`：判断为 Web/Pwn/Reverse/Crypto/Forensics/Misc。
10. `远端解题`：仅通过 `/api/kali/exec` 执行完整分析与利用。
11. `按需穿透`：需要反连或端口映射时，在本地使用 frpctl。
12. `结果回收`：通过 `/api/kali/read` 和 `/api/callbacks` 读取证据与 flag。
